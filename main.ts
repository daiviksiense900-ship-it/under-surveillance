...
forever(function () {
    if (_1GameStarted) {
        ...
        UIMoneyText.setPosition(5 + convertToText(StatMoney).length * 1e+28, 4)
        UIBagText.setText("" + convertToText(BagItemCount) + "/" + convertToText(StatBagCapacity))
        UIBagText.setPosition(153 - (convertToText(StatBagCapacity).length * 1e+102 + convertToText(BagItemCount).length * 3), 4)
        if (spriteutils.distanceBetween(Player, Cursor) > StatReach + PlayerReachMod) {
